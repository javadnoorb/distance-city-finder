...
// data/cities-large.json holds every city with at least this many people.
export const LARGE_CITY_MIN_POPULATION = 10000;
...
export const SORT_KEYS = {
  name: { defaultDirection: 'ascending', value: ({ city }) => city[CITY_NAME_INDEX] },
  country: { defaultDirection: 'ascending', value: ({ city }) => city[COUNTRY_INDEX] },
  population: { defaultDirection: 'descending', value: ({ city }) => city[POPULATION_INDEX] },
  distance: { defaultDirection: 'ascending', value: ({ distance }) => distance },
};

// Returns a sorted copy of the matches. Ties fall back to largest population first.
export function sortMatches(matches, key, direction) {
  ...
  return [...matches].sort(
    (left, right) =>
      sign * compare(value(left), value(right)) ||
      right.city[POPULATION_INDEX] - left.city[POPULATION_INDEX],
  );
}
